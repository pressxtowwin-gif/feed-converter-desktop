...
# -*- coding: utf-8 -*-
"""
Универсальный локальный конвертер XML-фидов недвижимости в Excel.

Поддерживаемые форматы:
1) Яндекс.Недвижимость XML: <realty-feed> ... <offer internal-id="...">
2) Домклик / Kvartirogramma XML: <complexes> ... <complex> ... <buildings> ... <flat>

Запуск из VS Code / терминала:
    python3 universal_feed_to_excel_converter.py --source feed.xml --output lots.xlsx

Запуск по ссылке:
    python3 universal_feed_to_excel_converter.py --source "https://example.com/feed.xml" --output lots.xlsx
...
def write_xlsx(rows: list[dict[str, object]], columns: list[str], output: str) -> None:
    wb = Workbook()
    ws = wb.active
    ws.title = "Лоты"
    ...
    ws.append(columns)
    for row in rows:
        ws.append([normalize_cell_value(row.get(col, "")) for col in columns])
    ...
    for idx, col_name in enumerate(columns, start=1):
        ...
        if col_name.startswith("Изображение "):
            ...
            ws.column_dimensions[letter].width = width_by_name.get(col_name, 18)

    for idx, col_name in enumerate(columns, start=1):
        if col_name in {"Цена", "Цена со скидкой"}:
            for col_cells in ws.iter_cols(min_col=idx, max_col=idx, min_row=2):
                for c in col_cells:
                    c.number_format = '#,##0'
        if col_name == "Общая площадь":
            for col_cells in ws.iter_cols(min_col=idx, max_col=idx, min_row=2):
                for c in col_cells:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Универсальный конвертер XML-фидов Яндекс.Недвижимость и Домклик/Kvartirogramma в Excel"
    )
    parser.add_argument("--source", required=True, help="Путь к XML-файлу или URL XML-фида")
    parser.add_argument("--output", required=True, help="Путь для сохранения XLSX")
    args = parser.parse_args()

    xml_bytes = load_xml(args.source)
    rows, columns, feed_format = parse_universal(xml_bytes)
    write_xlsx(rows, columns, args.output)

    print(f"Готово: {args.output}")
    print(f"Формат фида: {feed_format}")
    print(f"Квартир обработано: {len(rows)}")
